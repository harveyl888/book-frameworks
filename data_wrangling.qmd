...
```{r}
#| eval: false
expr <- rlang::call2("+", rlang::call2("*", rlang::sym("mpg"), rlang::sym("cyl"), 5))
```

To include this in a JSON expression, we can consider recursively breaking the entire expression into smaller and smaller expressions comprising of an operator, a left-hand side and a right-hand side.  As an instruction set this could look like:
...
The solution contains a recursive function which keeps calling itself on the left and right hand side operations until they result in a column name or value.

This method is not fool-proof and works for simple, general cases.  Ideally, the logic should be constructed so that anything representing a *name* is returned as a symbol and anything else should be returned as a literal.  In order to achieve this we need to know when a string is a *name* and when it is a string.  For example - it would be impossible for this code to determine if "mpg" refers to the column called mpg or a string "mpg".  One solution would be to specify in the JSON format itself, for example:
