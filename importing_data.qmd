...
Importing data is a fundamental step for a data processing framework.  It's also quite simple due to the many functions in base R and R packages available for importing.  In this chapter we'll explore how an R-based framework can be used to import data.

## A Simple Example

We will start with a simple example.  Our engine will simply read in data and print it to the console.  For the data we'll use the cricket dataset from TidyTuesday.
...
This file contains just a single parameter, `dataset`, pointing to the URL of the dataset to read.  Our engine has a very simple purpose - read in the file and send the output to the console.  Our engine looks as follows:
...
We have one parameter in our json file, `dataset`.  Changing it will change the dataset imported and printed.  Of course, if we had written an R script we could simply change the filename in the script itself or change a variable pointing to the script but using an external instruction set allows us to change the parameter without affecting the script itself.  For example, if we want chocolate ratings instead of cricket statistics we simply change the dataset parameter:
...
Using the same engine outputs the following:
...
Our engine (which is very simple) can be used to read in and display either dataset.  Later chapters will explore how we can use a single engine to perform data manipulation and graphing for a number of different data imports.

## Importing Different Data Formats

By introducing a conditional, an engine can import different data types.  

### Introducing a parameter to specify the file type

In the most simple approach we can add a parameter to specify the dataset type and then respond accordingly.  For example adding a `format` parameter to our instruction set and responding to its value:
...
Thus, our instruction set could revert back to the shorter version in which the file type is not specified:
...
#| cache: true
#| warning: false
out_read_all <- purrr::map(seq(2), \(i) {
  f <- sprintf("data/f_%02i.csv", i)
  bench::mark(
    `read.csv` = read.csv(f),
...
df <- tidyr::unnest(out_read_all[[1]], c(time, gc)) |> 
  ...
  dplyr::mutate(ms = as.numeric(time, units = "ms"))
ggplot2::ggplot(df, ggplot2::aes(ms, expression)) + 
  ggplot2::geom_violin() + 
...
This knowledge can be carried over to our engine.  For example, the engine code could be updated as follows so that only necessary columns are imported:
