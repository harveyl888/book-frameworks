...
::: {.callout-warning}
The UI is built using `renderUI()` but the `observeEvent()` functions are not.  Observers should not be tied to `renderUI()` as there is a danger that they can be built more than once.
:::
...
-  Shiny does not store the type of an input.  A workaround would be to use the type of widget in its name (for example slider_age).
